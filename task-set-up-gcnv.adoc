...
NOTE: Le modifiche alle autorizzazioni dei progetti Google Cloud potrebbero richiedere un paio di minuti per avere effetto.
...
.. Dall'elenco dei ruoli, seleziona il ruolo *Google Cloud NetApp Volumes Admin* o *Google Cloud NetApp Volumes Viewer*.
...
NOTE: Per gli ambienti di staging, utilizzare  `credentials-sa@wf-staging-netapp.iam.gserviceaccount.com` invece dell'account di servizio di produzione. Sostituire il valore del parametro  `--member` nel comando precedente con  `serviceAccount:credentials-sa@wf-staging-netapp.iam.gserviceaccount.com`.
